...
parser.add_argument('--max_bitwidth', action='store_true',
...
print('Infering SSD using the specified args:')
print(args)
f = open("param_infer.txt", "w")
f.write(str(args)+"\n")
f.close()
...
def do_python_eval(output_dir='output', use_07=True):
    ...
    if os.getenv('AVG_LOG'):
        with open(os.getenv('AVG_LOG'), 'a') as eval_out:
            eval_out.write('mAP:{}\n'.format(np.mean(aps)))
...
if __name__ == '__main__':
    ...
    net.load_state_dict(torch.load(args.trained_model, map_location=torch.device('cpu')), strict=False)
    net.eval()
    ...
    # evaluation
    test_net(args.save_folder, net, args.device, dataset,

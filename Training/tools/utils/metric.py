...
adaptive_cnt = int(adaptive_strategy_env) if (adaptive_strategy_env is not None) else 0
...
class ElapsedTimer(object):
    def __init__(self, count_down=0):
        self.meter = AggregatorMeter(
                AverageMeter("batch_time_avg"), 
...
class HardwareTimer(object):
    ...
    def clock(self):
        ...
        hardware_time = self.start_notifier.hardware_time(self.end_notifier) / 1000.0 / 1000.0
...
class MetricCollector(object):
    def __init__(
        ...
        self._enabled = self.check_enable(enable, enable_only_benchmark, enable_only_avglog) 
    ...
    def _init_recorders(self):
        if self.record_elapsed_time:
            self._recorders.append(
                    ElapsedTimer(count_down=adaptive_cnt))
        if self.record_hardware_time:
            self._recorders.append(
                    HardwareTimer(count_down=adaptive_cnt))
    ...
    def insert_metrics(self, **kwargs):
        self._insert_metrics = kwargs

    def update_recorder_metrics(self):
        for recorder in self._recorders:
            result = recorder.data()
            if len(result) == 0:
                print("MetricCollector have not recorded any datas, please ensure \
